...
from signjoey.data import load_data, make_data_iter, load_test_data
...
def prepare_model(cfg, gls_vocab, txt_vocab, ckpt: str, bit=False):
...
def prediction(
    model, gls_vocab, txt_vocab, sequence_field, signer_field, sgn_field, gls_field, txt_field, cfg_file, ckpt: str, output_path: str = None, logger: logging.Logger = None, bit=False
) -> None:
    ...
    #model = prepare_model(cfg, gls_vocab, txt_vocab, ckpt, bit)
    ...
    # Load Testset
    test_data = load_test_data(cfg["data"], sequence_field, signer_field, sgn_field, gls_field, txt_field)
    ...
    def _write_to_file(file_path: str, sequence_ids: List[str], hypotheses: List[str]):
        ...
        for seq, hyp in zip(sequence_ids, hypotheses):
            #out_file.write(seq + "|" + hyp + "\n")
            translation_outputs.append(hyp)
    ...
    if output_path is not None:
        ...
        if do_translation:
            ...
            translation_outputs = _write_to_file(
                test_txt_output_path_set,
                [s for s in test_data.sequence],
                test_best_result["txt_hyp"],
            )
